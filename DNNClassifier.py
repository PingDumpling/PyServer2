...
class BPNN:
    def __init__(self, nn_shape=[2, 4, 1]):     # 输入层中具有2个变量的网咯，有一个具有8个节点的隐藏层，和具有一个节点的输出层
        self.W = []  # 权重
        self.B = []  # 阈值
        self.O = []  # 各神经元节点输出
...
path1 = r'D:\TestFile\merge.csv'
rawData = pd.read_csv(path1).values  # rawData--读取CSV文件数据到rawData中
X = rawData[:, 0:4]
Y = rawData[:, 4]
...
bp = BPNN([4, 8, 4, 4])            # 建立神经网络对象
Y = bp.one_hot_label(Y)



x_train, x_test, y_train, y_test = train_test_split(X, Y, test_size=0.35)  # 划分训练集和测试集


bp.fit(x_train, y_train, Preprocess=True, thre=0.01, learning_rate=0.001, max_iter=800)      # 构建网络
...
print('准确率：{}'.format(count / y_test.shape[0]))
plt.plot(bp.errs)
plt.show()
